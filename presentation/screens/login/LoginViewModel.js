...
const LoginViewModel = () => {
    ...
    const [state, setState] = useState({ phone: '', showCode: false, navigateToHome: false })
    ...
    const loginAndVerify = async (code) => {
        ...
        if (!verifyId)
            return
        setLoading({ isLoading: true })
        const { authData } = await authRepository.loginAndVerify(verifyId, state.phone, code)
        dispatch({
            type: 'LOGIN',
            payload: {
                user: authData.user,
    ...
    return { ...state, hideCode, onPhoneChanged, sendAuthVerification, loginAndVerify }
}
